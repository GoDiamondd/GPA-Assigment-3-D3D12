...
int main() {
    ...
    // One-time grass upload (OBJ/MTL + texture upload)
    g_alloc[g_frameIndex]->Reset();
    g_cmdList->Reset(g_alloc[g_frameIndex].Get(), nullptr);

    if (!renderer->initVegetationAssets(g_cmdList.Get()))
    {
		std::cout << "Grass initVegetationssAssets failed.\n";
    }
    //renderer->initGrassField(g_cmdList.Get(), /*halfSize*/ 50.0f, /*spacing*/ 0.5f);
    //renderer->initGrassFieldFromPoisson(g_cmdList.Get(), "assets/models/spatialSamples/poissonPoints_155304s.ss2", 0.0f, 1.0f);
    renderer->initAllVegetationFieldsFromPoisson(g_cmdList.Get(), /*y=*/0.0f, /*scale=*/1.0f);
    g_cmdList->Close();
    ID3D12CommandList* initLists[] = { g_cmdList.Get() };
