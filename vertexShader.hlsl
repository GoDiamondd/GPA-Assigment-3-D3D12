...
// Keep b2 for the non-instanced drawGrass() path.
// For indirect/instanced path we will ignore it.
cbuffer ModelConstants_T : register(b2)
{
    column_major float4x4 modelMat_T;
};

// NEW: instance transforms buffer (matches RendererBase::initGrassField SRV at t1)
StructuredBuffer<float4x4> g_instances : register(t1);
...
struct VS_OUTPUT_TEX
{
    ...
    float3 viewPos : TEXCOORD3; // NEW (view-space position)
};

VS_OUTPUT_TEX VSMainTextured(VS_INPUT_TEX input, uint iid : SV_InstanceID)
{
    VS_OUTPUT_TEX o;

    float4x4 model = g_instances[iid];

    float4 w = mul(model, float4(input.v_vertex, 1.0f));
    float3 n = mul((float3x3) model, input.v_normal);

    o.worldPos = w.xyz;
    o.worldN = normalize(n);

    float4 vpos = mul(viewMat_T, w);
    o.viewPos = vpos.xyz; // NEW

    o.pos = mul(projMat_T, vpos);
    o.uv = input.v_uv;
    return o;
}